...
Every element source (Space-Track, Celestrak) implements ``SourceAdapter`` so the
scheduler and the merge layer treat them identically. New feeds (P4b:
JCO/ITU/UNOOSA) drop in by adding another adapter — nothing upstream changes.
This is the seam the dev plan (Stage 2) calls for: "keep source adapters uniform
so new feeds drop in later." (DISCOS is handled separately as a metadata
enrichment pass — it carries no orbital state — see ingestion/discos.py.)
...
class SourceAdapter(ABC):
    ...
    def available(self) -> bool:
        ...
        Default-on; sources that need credentials (e.g. Space-Track) override
        this to gate on configuration so the scheduler skips them cleanly when
        unconfigured.
        """
        return True
